...
public class MerchantMutationIntegrationTest {
    ...
    void createMerchant() {
        ...
        assertEquals(merchant.getId(), productMutation.getProductService().findProductById(product.getId()).getMerchant().getId());
        assert userMutation.getUserService().findUserById(user.getId()).getMerchants().contains(merchant);
    }
    ...
    void deleteMerchant() {
        ...
        assert productMutation.getProductService().findProductById(product.getId()).getMerchant() == null;
        assert !userMutation.getUserService().findUserById(user.getId()).getMerchants().contains(merchant);
    }
    ...
    void updateMerchant() {
        ...
        UserInput userInputUpdate = new UserInput("t", "t", "t.t@g.c", "F", "F", "31450", "c", null, null);
        ...
        Merchant merchantUpdated = merchantMutation.updateMerchant(merchant.getId(), merchantInputUpdate);
